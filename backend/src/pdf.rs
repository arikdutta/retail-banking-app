...
pub fn build_source(
    ...
    let rows: String = transactions
        ...
        .map(|t| {
            ...
            let sign = if t.amount >= rust_decimal::Decimal::ZERO {
                "+"
            } else {
                ""
            };
...
mod tests {
    ...
    use crate::domain::transactions::model::{
        Transaction, TransactionCategory, TransactionStatus, TransactionType,
    };
    ...
    fn source_contains_email_and_period() {
        ...
        assert!(
            source.contains("alice@example.com") || source.contains("alice\\@example.com"),
    ...
    fn source_prefixes_positive_amount_with_plus() {
        ...
        assert!(
            source.contains("+3200.00"),
            "positive amounts get '+' prefix"
        );
    }
