...
async function fetchInvoices(page: number): Promise<InvoicesPage> {
  ...
  if (!r.ok) throw new HttpError(r.status, `Failed to fetch invoices: ${r.status}`);
  return r.json();
}
...
async function createInvoice(body: CreateInvoiceRequest): Promise<Invoice> {
  ...
  if (!r.ok) {
    ...
    throw new HttpError(r.status, (err as { error?: string }).error ?? `Failed to create invoice: ${r.status}`);
  }
  return r.json();
}
...
async function updateInvoiceStatus(id: string, status: string): Promise<Invoice> {
  ...
  if (!r.ok) {
    const err = await r.json().catch(() => ({}));
    throw new HttpError(r.status, (err as { error?: string }).error ?? `Failed to update status: ${r.status}`);
  }
  return r.json();
}
